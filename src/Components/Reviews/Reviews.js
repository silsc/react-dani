...
function Reviews() {
  ...
  function handleTouchStart(e) { setTouchStart(e.targetTouches[0].clientX); }

  function handleTouchMove(e) { setTouchEnd(e.targetTouches[0].clientX); }
  ...
  return (
    ...
    htmlFor = {"t-" + (index + 1)} id = {"testimonial-" +
                                         (index + 1)} onTouchStart =
        {touchStartEvent => handleTouchStart(
             touchStartEvent)} onTouchMove = {touchMoveEvent =>
                                                  handleTouchMove(
                                                      touchMoveEvent)} onTouchEnd =
            {() => handleTouchEnd(index)} >
            <img src = {testimonial.image} className = 'testimonial-img' />
            <div className = "testimonial-container">
            <p className = "testimonial-text">{testimonial.text}<
                /p>
                      <h1 className="testimonial-name">{testimonial.name}</h1>
            <h3 className = "testimonial-band">{
                testimonial.band}</h3>
                    </div>
            </label>
                );
